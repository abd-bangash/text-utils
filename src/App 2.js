...
// all methods needed to be passed as props to children components
  ...
  const startSpeechRecognition = () => {
    ...
    recognition.start();
    manageAlertObjectForSpeech("Speak into mic , once you start speaking pausing for 2 seconds will stop recording the message","info")
  }
  const clearText=()=>{
    setText("");
  ...
  return (
    <>
    <Navbar title='TextUtils...' about='About' mode={mode} handleMode={handleMode} handlePalette1={handlePalette1} handlePalette2={handlePalette2}handlePalette3={handlePalette3}handlePalette4={handlePalette4} ></Navbar>
    <Alert alert={alert} />
    <TextArea heading="Enter the text below....." text={text} palette={palette} handleTextValueChange={handleTextValueChange} mode={mode}  ></TextArea>
    <Butons handleUpperCaseClick={handleUpperCaseClick} handlelowerCaseClick={handlelowerCaseClick} speak={speak} startSpeechRecognition={startSpeechRecognition} clearText={clearText} handleCopy={handleCopy} handleExtraSpaces={handleExtraSpaces}></Butons>
    <Summary text={text} mode={mode} palette={palette}></Summary>
    <About mode={mode} palette={palette}></About>
    </>
  );
}
